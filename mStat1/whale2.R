...
# solve the nonlinear equation (suugested by Jacob Colvin)

lik = function(p) { 
	-(n*p[1]*log(p[2])+(p[1]-1)*sum(log(whale))-p[2]*sum(whale)-n*log(gamma(p[1]))) 
}

gamma.mle <- nlminb(start=c(1,1),lik,lower=c(0,0)) 

alpha.hat.mle = gamma.mle$par[1]
alpha.hat.mle

lambda.hat.mle = gamma.mle$par[2]
lambda.hat.mle
...
for(i in 1:B){
	...
	alpha.star.mle[i] = gamma.mle$par[1]		# alpha
	lambda.star.mle[i] = gamma.mle$par[2]	# lambda
}
